...
export const Tickets = () => {
    ...
    useEffect(() => {
        const getTicketsByUser = async () => {
            ...
            await axios.post('http://localhost:5050/api/data', data).then(res => setUserTickets(res.data.tickets.ticket));
        }
        getTicketsByUser();
    }, [userMail])

    const getClientTickets = async (clientId: number | string) => {
        const data = {
            clientId: clientId,
            action: 'GetTickets'
        }
        ...
        <div className='container-fluid'>
             <div className="row flex-nowrap">
                ...
                <div className="container-sm col">
                    <h1 className="text-center">Tickets For Client (Inactive)</h1>
                    <ul>
                        {clients && clients.map((client, index) => {
                            ...
                            <li className="d-flex align-items-center justify-content-evenly my-2" key={index}>
                                ...
                                &nbsp;
                                <Button size="sm" variant="secondary" onClick={() => getClientTickets(client.id)}>Get Tickets</Button>
                            </li>
                            )
                        })}
                    </ul>
                    ...
                    <ul>
                        {userTickets && (userTickets as any)?.map((ticket: any, index: React.Key | null | undefined) => {
                            ...
                            <li className="d-flex align-items-center flex-column justify-content-evenly my-2" key={index}>
                                <Link to={`/tickets/${ticket.id}`}><h6 className="mb-0">Ticket ID: #{ticket.id}</h6></Link>
                            </li>
                            )
                        })}
                    </ul>
